...
opam exec -- opam-publish \
  ...
  --no-browser \
  "https://github.com/formalsec/ocaml-cvc5/releases/download/$TAG/ocaml-cvc5-$TAG.tar.gz" | tee "$OUTPUT_FILE"
...
if [ -z "$PR_LINK" ]; then
  ...
  if [[ -n "${SLACK_WEBHOOK_URL}" ]]; then
    ...
    echo "🚀 *ocaml-cvc5* version *$TAG* has been submitted to opam-repository! *PR:* $PR_LINK" | sigh notify "$SLACK_WEBHOOK_URL"
